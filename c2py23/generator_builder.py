...
def generate(module_def):
    ...
    # Header
    b.emit('/* Generated by c2py23 builder - do not edit by hand */')
...
def _emit_c_call_builder(b, ol, buf_params, scalar_params,
    ...
    for p in ol.params:
        ...
        if p.is_pointer and _is_ptr_expr(expr):
            ...
            arg_c = '(' + p.ctype + ')(' + arg_c + ')'
        # Check for INT_MAX overflow
        if not _expr_is_count_or_len(expr):
            args.append(arg_c)
        else:
            emitl('if (({0}) > (Py_ssize_t)INT_MAX) {{'.format(arg_c))
    ...
    if gil_release_call:
        b.gil_save('_c2py_do_gil', '_c2py_thread_state')
    ...
    if not has_outputs:
        ...
        if gil_release_call:
            b.gil_restore('_c2py_do_gil', '_c2py_thread_state')
    ...
    if n == 1:
        ...
        for i, (name, ctype, val) in enumerate(out_items):
            if ctype in ('int', 'int8_t', 'int16_t', 'int32_t',
                         'uint8_t', 'uint16_t', 'uint32_t'):
                b.emit_output_long(i, val)
            elif ctype == 'int64_t':
                b.emit_output_long(i, val, 'int64_t')
            elif ctype == 'uint64_t':
                b.emit_output_long(i, val, 'uint64_t')
            elif ctype in ('float', 'double'):
                b.emit_output_double(i, val)
            else:
                b.emit_output_double(i, val)
