...
/* ------------------------------------------------------------------ */
/* Multi-source buffer acquisition: ndarray fast path, DLPack, buffer */
/* ------------------------------------------------------------------ */

/* Source identifiers for c2py_acquire ordering */
#define C2PY_SRC_NDARRAY  0
#define C2PY_SRC_DLPACK   1
#define C2PY_SRC_BUFFER   2
#define C2PY_MAX_SRC      3

/* Minimal PyTypeObject: just the first few fields we need for tp_name.
 * Layout (LP64): ob_refcnt(8) + ob_type(8) + ob_size(8) + tp_name(8). */
typedef struct {
    Py_ssize_t ob_refcnt;
    void     *ob_type;
    Py_ssize_t ob_size;
...
/* Runtime-discovered numpy ndarray struct layout.
 * Fields at the front of PyArrayObject have been stable since
 * numpy 1.0 through 2.x.  Only data_off varies (PyObject_HEAD
 * size differs between GIL and free-threaded builds). */
typedef struct {
    void *ndarray_type;           /* cached ob_type pointer */
    int data_off;                 /* offset of data pointer from PyObject base */
    int probed;                   /* 1 if layout has been discovered */
    char format_buf[2];           /* {type_char, 0}, filled per-call */
} c2py_ndarray_layout_t;

extern c2py_ndarray_layout_t C2PY_NDARRAY;

/* Try to fill buf from a numpy ndarray via direct struct cast.
 * Returns 0 on success, -1 if obj is not a known ndarray (fall through). */
static inline int
c2py_try_ndarray(PyObject *obj, Py_buffer *buf, int want_writable)
{
        ...
        if (name && strcmp(name, "ndarray") == 0) {
            ...
            base = (char*)obj;
            {
                ...
                for (off = (int)C2PY.pyobject_size;
                     off < (int)C2PY.pyobject_size + 80; off += (int)sizeof(void*)) {
                    if (*(void**)(base + off) == buf->buf) {
                        L->data_off = off;
                        break;
                    }
                }
...
fill:
    ...
    buf->buf   = dptr;
    buf->ndim  = nd;
    buf->shape = *(Py_ssize_t**)(base + L->data_off + 16);
    buf->strides = *(Py_ssize_t**)(base + L->data_off + 24);
    buf->suboffsets = NULL;
    buf->internal = NULL;
    buf->readonly = want_writable ? 0 : 1;
    buf->obj = NULL;  /* no PyBuffer_Release needed for ndarray fast path */

    descr = *(void**)(base + L->data_off + 40);
    if (descr) {
        /* type char is at stable offset 25 within PyArray_Descr (since numpy 1.0).
         * Itemsize is derived from the type char rather than reading descr->elsize,
         * since numpy 2.0 changed the elsize field offset. */
        type_char = ((char*)descr)[25];
        L->format_buf[0] = type_char;
        L->format_buf[1] = '\0';
        buf->format = L->format_buf;
        buf->itemsize = 0;
        switch (type_char) {
            case 'b': case 'B': case '?': buf->itemsize = 1; break;
            case 'h': case 'H':          buf->itemsize = 2; break;
            case 'i': case 'I':          buf->itemsize = 4; break;
            case 'l': case 'L':          buf->itemsize = (int)sizeof(long); break;
            case 'q': case 'Q':          buf->itemsize = 8; break;
            case 'f':                    buf->itemsize = 4; break;
            case 'd':                    buf->itemsize = 8; break;
            case 'g':                    buf->itemsize = (int)sizeof(long double); break;
            default:                     buf->itemsize = 1; break;
        }
    } else {
        buf->format = NULL;
        buf->itemsize = 1;
    ...
    /* len = total bytes: product of shape * itemsize */
    {
        Py_ssize_t nelem = 1;
        int i;
        for (i = 0; i < nd && buf->shape != NULL; i++)
            nelem *= buf->shape[i];
        buf->len = nelem * (Py_ssize_t)(buf->itemsize > 0 ? buf->itemsize : 1);
    }
 ...
 * Tries sources in src_order, using last_src as a hint for the first try.
 * Returns 0 on success, -1 on failure (with Python exception set).
 * Thread-safe: stale last_src is a benign hint; worst case is one extra
 * failed probe before the ordered walk finds the correct source. */
static inline int
c2py_acquire(PyObject *obj, Py_buffer *buf, int want_writable,
             const uint8_t *src_order, int n_src, int *last_src)
{
    int i, cached;

    memset(buf, 0, C2PY.pybuffer_size);

    /* Phase 1: try the hint from last call */
    cached = *last_src;
    if (cached >= 0 && cached < n_src) {
        switch (src_order[cached]) {
        case C2PY_SRC_NDARRAY:
            if (c2py_try_ndarray(obj, buf, want_writable) == 0) return 0;
            break;
        case C2PY_SRC_DLPACK:
            break;  /* not yet implemented */
        case C2PY_SRC_BUFFER:
            if (c2py_acquire_buffer(obj, buf, want_writable) == 0) return 0;
            break;
        }
    }

    /* Phase 2: ordered walk over all sources */
    for (i = 0; i < n_src; i++) {
        switch (src_order[i]) {
        case C2PY_SRC_NDARRAY:
            if (c2py_try_ndarray(obj, buf, want_writable) == 0) {
                *last_src = i; return 0;
            }
            break;
        case C2PY_SRC_DLPACK:
