...
| File | Contents |
|------|----------|
| `audit/full_repo_audit.md` | Every tracked text file, grouped by component |
| `audit/wrappers_combined.md` | All wrapper modules + runtime header (one file) |
| `audit/wrappers/<name>.md` | One file per module (runtime NOT included) |
...
python3 audit/export_full_repo.py   # full source audit
python3 audit/export_wrappers.py    # wrapper module audit
```

Both scripts refuse to run if `git status --porcelain` is not clean
(untracked `audit/` files are allowed).  Submodules (`examples/kissfft`,
...
--output-dir DIR    Write output to DIR (default: audit/)
```
...
`docs/referee_reports_2026-06-15.md`.  The workflow below can be repeated
for each new revision of the code.
...
Upload each file from `audit/wrappers/<name>.md` sequentially.  Each file
contains the YAML interface definition, the C implementation, the generated
wrapper, and any test scripts for that module.
...
The previous review used a summary table (see
`docs/referee_reports_2026-06-15.md` lines 17--33).  Use the same format.
...
For a holistic review, upload `audit/full_repo_audit.md` instead.  This
contains the Python generator source (parser, generator, CLI) alongside
the C runtime, so the LLM can review the entire pipeline end-to-end.
