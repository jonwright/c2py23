...
Options:
    ...
    --output-dir DIR    Output directory (default: tools/audit/)
...
def _git_clean_check():
    ...
    try:
        out = subprocess.check_output(["git", "status", "--porcelain"], cwd=REPO_DIR, stderr=subprocess.STDOUT).decode(
            "utf-8", errors="replace"
        )
    except subprocess.CalledProcessError:
        return True
    lines = [l for l in out.splitlines() if l.strip() and not l.startswith("?? tools/audit/")]
    if lines:
...
def _git_ls(dir_path):
    ...
    try:
        out = subprocess.check_output(["git", "ls-files", dir_path], cwd=REPO_DIR, stderr=subprocess.STDOUT).decode(
            "utf-8", errors="replace"
        )
    except subprocess.CalledProcessError:
        return []
...
def _find_modules():
    ...
    for top_dir in ["tests/cases", "examples"]:
        ...
        for entry in sorted(os.listdir(top)):
            ...
            c2py_files = [f for f in os.listdir(mod_dir_full) if f.endswith(".c2py")]
            if not c2py_files:
                continue
...
def _module_files(mod_dir_rel):
    ...
    wrappers = [f for f in os.listdir(mod_dir_abs) if f.endswith("_wrapper.c")]
    wrapper_rel = None
    wrapper_abs = None
    if wrappers:
    ...
    # Filter out binary
    tracked = [f for f in tracked if _is_text(f) and not f.endswith("_wrapper.c")]
...
def _run(cmd, cwd=None, env=None):
    try:
        proc = subprocess.Popen(cmd, shell=True, cwd=cwd, env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def _build_check(skip_build):
    ...
    for mod_dir, c2py_rel, name in mods:
        ...
        if not os.path.isfile(so_abs):
            sos = [f for f in os.listdir(os.path.join(REPO_DIR, mod_dir)) if f.endswith(".so")]
            if sos:
    ...
    if failed:
        print("\nERROR: {} module(s) failed to build: {}".format(len(failed), ", ".join(failed)))
        return False
...
def _test_check(skip_tests):
    ...
    for label, script in [
        ...
        ret, out, err = _run('{} "{}"'.format(py, os.path.join(test_dir, script)))
        if ret != 0:
    ...
    sif_files = [f for f in os.listdir(snakepit_dir) if f.endswith(".sif")] if os.path.isdir(snakepit_dir) else []
    if sif_files:
...
def _export_combined(modules, output_dir):
    ...
    py_ver = "{}.{}.{}".format(sys.version_info[0], sys.version_info[1], sys.version_info[2])
...
def _export_individual(modules, output_dir):
    ...
    for mod_dir, c2py_rel, name in modules:
        section_text, files_included = _emit_module_section(mod_dir, c2py_rel, name)
...
def main():
    ...
    modules = [(m, c, n) for m, c, n in all_mods if os.path.basename(m) not in _SKIP_MODULES]
