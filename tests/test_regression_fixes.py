...
def test_generated_wrapper_deterministic_and_versioned():
    """Generated wrappers are byte-identical across runs and carry the
    c2py23 version so a committed wrapper can be traced to its generator
    (drift detection; see jonwright/c2ImageD11#34).
    """
    from c2py23 import __version__

    here = os.path.dirname(os.path.abspath(__file__))
    mod = load_c2py(os.path.join(here, "cases", "transform", "transform.c2py"))
    code1 = generate(mod)
    code2 = generate(mod)
    assert code1 == code2, "generate() output must be deterministic across runs"

    first = code1.split("\n")[0]
    assert "Generated by c2py23 v%s" % __version__ in first, first
    assert "do not edit by hand" in first

    # restrict alias checks must come out in sorted buffer-name order
    idx = code1.find("restrict check:")
    assert idx != -1
    first_check = code1[idx:].split("\n")[0]
    assert "out vs points" in first_check, "checks must be sorted, got: %s" % first_check
