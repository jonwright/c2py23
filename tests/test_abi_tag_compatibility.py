...
def test_build_wheel(tmpdir):
    ...
    if not so_files:
        raise AssertionError("No %s file produced. Available files: %s" % (ext, os.listdir(src_dir)))
    ...
    # Build the wheel
    rc, out, err = _run([sys.executable, "setup.py", "bdist_wheel"], cwd=project)
    if rc != 0:
    ...
    assert tags["py_tag"] == "py3", "Expected py_tag='py3', got %s" % tags["py_tag"]
    assert tags["abi_tag"] == "none", "Expected abi_tag='none', got %s" % tags["abi_tag"]
    assert tags["plat_tag"] == "any", "Expected plat_tag='any', got %s" % tags["plat_tag"]

    print("[OK] Wheel ABI tags: %s" % wheel_path)

    # 2. Verify the wheel contains a compiled .so file
    assert _wheel_contains_extensions(wheel_path), "Wheel %s contains no .so/.pyd files -- unexpectable." % wheel_path
    ...
    if rc != 0:
        ...
        if "not a supported wheel" in err.lower() or "platform" in err.lower():
    ...
    rc2, out2, err2 = _run(
        [
            sys.executable,
            "-c",
            'import sys; sys.path.insert(0, "%s"); import mysum; ' 'print(mysum.mysum.__doc__ or "")' % install_dir,
        ]
    )
    if rc2 != 0:
...
def test_abi_tag_naming_standards():
    ...
    for (major, minor), tags in sorted(standard_tags.items()):
        print("  Python %s.%s: GIL=%s FT=%s" % (major, minor, tags["gil"], tags["ft"]))

    # Verify that the abi3.abi3t compressed tag is recognized
    assert (
        standard_tags[("3", "15")]["ft"] == "cp315-abi3.abi3t"
